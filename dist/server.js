  ...
  "node_modules/pg/lib/client.js"(exports2, module2) {
    ...
    var Client2 = class extends EventEmitter {
      ...
      _handleReadyForQuery(msg) {
        ...
        const activeQuery = this._getActiveQuery();
        this._activeQuery = null;
        this._txStatus = (msg == null ? void 0 : msg.status) ?? null;
...
function statementTimeoutMs() {
  const raw = GetConvarInt("pg_statementTimeoutMs", 3e4);
  return raw > 0 ? raw : 3e4;
}
function connectionTimeoutMs() {
  const raw = GetConvarInt("pg_connectionTimeoutMs", 1e4);
  return raw > 0 ? raw : 1e4;
}
function createPool() {
...
async function connectWithRetry() {
  ...
  for (; ; ) {
    try {
      ...
      try {
        await (pool == null ? void 0 : pool.end());
      } catch {
      }
      pool = void 0;
...
function categoryOf(entry, slowMs) {
...
async function query(text, params = []) {
  return timed(text, async () => {
    ...
    return pool2.query(resolvedText, values);
  });
}
...
function splitParamsAndCallback(maybeParams, maybeCb) {
  if (typeof maybeParams === "function") {
    return { params: [], cb: maybeParams };
  }
  return { params: maybeParams ?? [], cb: maybeCb };
}
exports("query", (text, maybeParams, maybeCb) => {
...
exports("scalar", (text, maybeParams, maybeCb) => {
  const { params, cb } = splitParamsAndCallback(maybeParams, maybeCb);
  return withCallback(scalar(text, params), cb);
});
exports("execute", (text, maybeParams, maybeCb) => {
  const { params, cb } = splitParamsAndCallback(maybeParams, maybeCb);
  return withCallback(execute(text, params), cb);
});
exports("insert", (text, maybeParams, maybeCb) => {
  const { params, cb } = splitParamsAndCallback(maybeParams, maybeCb);
  return withCallback(insert(text, params), cb);
});
exports("batch", (statements, cb) => {
  return withCallback(batch(statements), cb);
});
exports("transaction", (fn, cb) => {
  return withCallback(transaction(fn), cb);
});
exports("tableExists", (table, cb) => {
  return withCallback(tableExists(table), cb);
});
exports("columns", (table, cb) => {
...
function canViewStats(playerSrc) {
